...
public class Drivetrain {
	...
	static final double ANGULAR_ADJUSTMENT_SCALE = 0.02;// motor speed per
														// degree
	...
	public boolean followCommand(Command2374 command) {
		...
		// computes the difference between the target angle/position and the
		...
		if (Math.abs(angleDifference) > 3) {
			...
			if (Math.abs(turnSpeed) > ANGULAR_ADJUSTMENT_MAX)
				turnSpeed = ANGULAR_ADJUSTMENT_MAX * Math.signum(turnSpeed);
			// 0.4 seems to be enough to overcome the carpet's friction
			if (Math.abs(turnSpeed) < 0.4)
				turnSpeed = 0.4 * Math.signum(turnSpeed);
		}
		// same algorithm for position adjustment
		if (Math.abs(posDifference) > 0.5) {
			speed = posDifference * AUTO_SPEED_SCALE;

			if (Math.abs(speed) > command.speed)
				speed = command.speed * Math.signum(speed);
			// 0.3 also seems sufficient
			if (Math.abs(speed) < 0.3)
				speed = 0.3 * Math.signum(speed);
		}
		...
		setMotorsQuadratic(speed + turnSpeed, speed - turnSpeed);// for Ian's
																	// practice
	...
	public int getEncoderAdjusted() {
		...
		// turning the robot in place changes the encoder's value, making this
		// necessary
		return encoder.get() + (int) (gyro.getAngle() / 6);
	}

	public void preciseTank(double lspeed, double rspeed) {
		// an experimental algorithm to make tank drive as good at going
		// straight/turning in place as arcade drive
		...
		setMotorsQuadratic(forwards + turn, forwards - turn); // for Ian's
																// practice
		// setMotors(forwards+turn,forwards-turn);
	}

	public void slowTank(double lspeed, double rspeed) {
		setMotors(quadraticScale(lspeed / 2), quadraticScale(rspeed / 2));
	}
	...
	// Ian wanted to have a drivetrain like last year's with speed ramping for
	// testing purposes
	...
	public void setMotorsQuadratic(double lspeed, double rspeed) {
		...
		// set speeds with signs as according to SetMotors method that Peter
		// wrote; PLEASE CHECK
		l1.set(-leftspeed);
		l2.set(-leftspeed);
		r1.set(rightspeed);
